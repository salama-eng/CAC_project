                    ...
                    <div class="modal-content  ">
                        <form action="{{ route('bid_auction',$auction->auction_post->id) }}" method="post">
                            ...
                            <div class="modal-body bg-darkgrey ">
                               @php
                                   $total=$auction->bid_total + $auction->auction_post->auction_ceiling;
                               @endphp
                                <h2 class="text-white fs-4 p-3"> المبلغ الحالي للسيارة <em class="yellow">{{$auction->bid_total }}$</em></h2>
                                <h3> يرجى اضافة مقدار الزيادة </h3>
                                <input type="number" step="{{$auction->auction_post->auction_ceiling}}" min="{{$auction->auction_post->auction_ceiling}}" value="{{$auction->auction_post->auction_ceiling}}" name="amount">
                            </div>
