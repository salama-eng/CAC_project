...
{{$column = $_GET['column'];}}
{{$name = $_GET['name'];}}
<h1 class="text-center fs-3 mb-3">تعديل المحتوى</h1>
<div class="container col-lg-8 col-11">
    @if(session()->has('error'))
        <div class="alert alert-success message">
            {{ session()->get('error') }}
    ...
    @endif
            <form action="{{ route('edit_content',$contentid) }}"  method="POST" enctype="multipart/form-data">
                ...
                <!-- Start content -->
                    ...
                    p-4">
                        <textarea name="{{$column}}" cols="30" class="form-control  mb-0"  style="border:none ;  background-color:transparent !important;  rows="7">
                        {{$Content[0]->$column}}
                        </textarea>
                ...
                <div class="mb-2 row">
                    <div class="offset-sm-2 col-sm-10">
                        <input type="submit" value="تعديل  النص" class=" btn p-2 contact m-auto">
                    </div>
                </div>
                <!-- End Submit -->
