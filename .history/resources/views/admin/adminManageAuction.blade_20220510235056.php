        ...
        <div class="table-responsive text-white ms-5">
            <table class="main-table manage-members text-center table table-bordered  text-white">
                ...
                @foreach($posts as $post)
                    @if($post->is_active == 1 && $apost->end_date >= date('Y-m-d') && $post->status_auction == 0)
                        <tr>
                            <td>{{$i++}}</td>
                            <td>{{$post->name}}</td>
                            <td>{{$post->auction->userOwner->name}}</td>
                            <td>{{$post->auction->userAw->name}}</td>
                            <td>{{$post->end_date}}</td>
                            <td>{{$post->auction->bid_amount}}</td>
                            <td>{{$post->auction->bid_total}}</td>
                            <td>
