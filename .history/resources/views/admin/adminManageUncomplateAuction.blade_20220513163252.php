                ...
                @foreach ($posts as $post)
                    ...
                    @if (isset($post->auctions[0]->post_id))
                        ...
                        @foreach($post->auctions as $auction)
                            @if($auction->post_id == $post->id && $auction->bid_total == $post->auctions->max('bid_amount'))
                                @php $user_id = $auction->userAw->id @endphp
                                @php $user_name = $auction->userAw->name @endphp
                                @php $bid_amount = $auction->bid_amount @endphp
                                @php $auction_id = $auction->id @endphp
                                @php $admin_confirm = $auction->admin_confirm @endphp
                                @php $user_confirm = $auction->user_confirm @endphp
                                @php $payment_confirm = $auction->payment_confirm @endphp
                            @endif
                            @if($auction->post_id == $post->id)
                                @php $total += $auction->bid_amount @endphp
                            @endif
                            @endforeach
                            <tr>
                                <td>{{$i++}}</td>
