    ...
    <section class="card-container  d-flex flex-wrap justify-content-between gap-3 ">
                    ...
                    @if ($post->is_active == 1)
                        ...
                        <div class="card text-light m-auto my-4" style="width: 18rem;">
                            ...
                            <div class="card-body d-flex justify-content-between py-0">
                                <p href="#" class="card-link card-details fs-7">سعر المزايدة/<span
                                        class="active">{{ $post->auctions->max('bid_total') }} $</span>
                                </p>
                            ...
                            <div class="card-body d-flex justify-content-between py-0">
                                ...
                                @if ($auction->user_confirm == 0)
                                    <a href="#" data-bs-toggle="modal" data-bs-target="#active{{ $post->id }} "
                                        class="card-link card-details fs-7 bg-yellow p-1 px-3 m-1 card-btn text-white">تاكيد
                                        الاستلام </a>
                                @else
                                    <p class="card-link card-details fs-7 p-1 px-3 m-1 card-btn text-white bg-green "><i
                                            class="fa fa-check"></i> تم الاستلام</p>
                                @endif

                                <a href="#" class="card-link card-details fs-7 bg-yellow p-1 px-3 m-1 card-btn text-white"
                                    data-bs-toggle="modal" data-bs-target="#payment{{ $post->id }}"> ارسال المبلغ </a>
                            </div>
                            <a href="{{ route('chat', $auction->id) }}"
                                class="card-link active bg-darkgrey p-1 card-btn px-3 m-1 text-center w-75 m-auto my-2 fs-7">
                                الذهاب للدردشة <i class="fa fa-long-arrow-left p-2 pt-1"> </i></a>
                        </div>

              

                        <div class="modal fade user" id="active{{ $post->id }}" tabindex="-1"
                                    ...
                                    <form action="user_confirm" method="post">
                                        ...
                                        <div class="modal-header">
                                            ...
                                            <button type="button" class="btn-close text-white" data-bs-dismiss="modal"
                                                aria-label="Close" style="color: white !important;"></button>
                                        </div>
                                        <div class="modal-body">
                                            <h2 class="text-white fs-5">هل انت متاكد انك استلمت سيارة {{ $post->name }}/
                                                {{ $post->model }}</h2>
                        ...
                        <div class="modal fade user" id="payment{{ $post->id }}" tabindex="-1"
                            aria-labelledby="exampleModalLabel" aria-hidden="true">
                            <div class="modal-dialog">
                                <div class="modal-content bg-dark">
                                    <form action="{{ route('testPayment') }}" method="get">
                                        ...
                                        <div class="modal-body">
                                            ...
                                            <input type="hidden" name="price"
                                                value="{{ $post->auctions->max('bid_total') }}">
        ...
        @if ($i <= 0)
            <div class="d-flex flex-wrap col-12 justify-content-center align-items-center text-center">
                ...
                <a href="{{ route('addAuction') }}" class="card-link active text-center mt-5 mb-2 col-12"> اضافة مزاد <i
                        class="fa fa-long-arrow-left p-2 pt-1"> </i></a>
