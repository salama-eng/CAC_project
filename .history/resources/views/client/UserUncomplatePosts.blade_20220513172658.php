...
<div class="card text-light m-auto my-4" style="width: 18rem;">
    ...
    <div class="card-body d-flex justify-content-between py-0">
        ...
        <a href="{{route('chat',$auction->id)}}" class="card-link active bg-darkgrey p-1 card-btn px-2 m-1 text-center w-75 m-auto my-2 fs-7"> الذهاب للدردشة  <i class="fa fa-long-arrow-left p-2 pt-1"> </i></a>
         ...
         </div>@if(isset($post->auction[0]->user_confirm ==0))
         <div class="d-flex red"><p class="card-link card-details fs-7 p-1 px-2 m-1 card-btn w-50 text-white">لم يتم الاستلام </p>
            @else
         <div class="d-flex"><p class="card-link card-details fs-7 bg-yellow p-1 px-2 m-1 card-btn w-50 text-white">لم يتم الاستلام </p>
