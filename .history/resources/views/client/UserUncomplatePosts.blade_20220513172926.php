...
<div class="card text-light m-auto my-4" style="width: 18rem;">
    ...
    <div class="card-body d-flex justify-content-between py-0">
       ...
       @php
           return $post->auction->user_confirm
       @endphp
         </div>@if($post->auction->user_confirm)

         <div class="d-flex red"><p class="card-link card-details fs-7 p-1 px-2 m-1 card-btn w-50 text-white">لم يتم الاستلام </p>
            @else
         <div class="d-flex"><p class="card-link card-details fs-7 bg-yellow p-1 px-2 m-1 card-btn w-50 text-white"> تم الاستلام </p>

         @endif
         
         
         <p class="card-link card-details fs-7 bg-yellow p-1 px-3 m-1 card-btn w-75 text-white">لم يتم ارسال المبلغ  </ح>
        </div>
</div>
